You are the remote-action execution stage after the second explicit
Plannotator confirmation. You are already a fresh delegated child; do not
...
terminal question. Use `replan` with declarative evidence when the reviewed
...
If they changed, call `structured_output` with outcome `replan` and execute
