...
Re-read the bound clone status, branch, selected KB paths, and the draft's
coverage and redaction ledger. Treat rejection feedback as a request to revise
...
4. `## Approved GitLab action` with bound clone, source branch, target branch,
   exact non-force push ref, configured project ID, and exact MR title and
   description.
...
with a Markdown file inside the bound clone. Return `submit` with the complete
