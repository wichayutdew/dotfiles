...
ruby -ryaml -e '
  ...
  assert publish_prompt.downcase.include?("returned description is empty"), "publish-remote.md: must block an empty GitLab default description"
  assert publish_prompt.include?("Update the same MR description through GitLab MCP"), "publish-remote.md: must update the resolved GitLab default description through MCP"
  assert publish_prompt.include?("only if the GitLab MCP update operation is unavailable or fails"), "publish-remote.md: must use glab only after GitLab MCP update failure"
  assert publish_prompt.include?("GitLab: use GitLab MCP update; use `glab mr update` only if the MCP update operation is unavailable or fails."), "publish-remote.md: must prefer GitLab MCP for existing-MR updates"
